...
// Importando as funções do nosso MiniGB APU (do arquivo C++)
extern uint8_t audio_read(uint16_t addr);
extern void audio_write(uint16_t addr, uint8_t val);
...
void gb_hw_write(unsigned a, byte b)
{
	...
	case 0xE000: // Memory mapped IO
		...
		{
			// Desvia os dados do jogo direto para o nosso APU super otimizado!
            audio_write(a, b);
            REG(a & 0xFF) = b;
		}
...
byte gb_hw_read(unsigned a)
{
	...
	case 0xE000: // Memory mapped IO
		...
		{
			// Corta a emulação nativa e le do APU!
            return audio_read(a);
		}
		// High RAM: 0xFF80 - 0xFFFE
